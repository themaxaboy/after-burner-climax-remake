...
export const LOOKS = {
  ...
  /** Storm front over a dark steel sea: low sun through heavy cloud (stage "storm"). */
  storm: {
    ...oceanDay,
    elev: 14,
    azim: -30,
    ...
    grade: 'arcadeStorm',
    toneExposure: 0.95,
    sunTint: [0.95, 1.0, 1.1],
    flare: 0.4,
    sky: sky({ saturation: 1.2, zenithBoost: -0.25, hue: [0.2, 0.42, 0.9], skyHue: 0.6, horizonHue: 0.5 }),
    bloom: { threshold: 3.0, smoothing: 2.5, intensity: 0.8 },
    clouds: { count: 1.5, minY: 180, maxY: 1100, spread: 2400, puffSize: [220, 480], sun: 0.8, ambient: 0.9 }
  },
  /** Black basalt ridges and red ash haze (stage "volcano", valley terrain). */
  volcano: {
    ...canyonRed,
    elev: 18,
    azim: 20,
    ...
    mie: 2.4,
    mieG: 0.86,
    groundAlbedo: [0.2, 0.08, 0.05],
    cloudCover: 0.2,
    fogDensity: 0.00014,
    horizonFog: 0.36,
    grade: 'arcadeVolcano',
    toneExposure: 1.0,
    sunTint: [1.12, 0.86, 0.7],
    flare: 0.5,
    sky: sky({ saturation: 1.35, zenithBoost: -0.1, hue: [1.0, 0.36, 0.16], skyHue: 0.65, horizonHue: 0.55, sunDisc: 2.2, sunGlow: 1.5 }),
    clouds: { ...canyonRed.clouds, count: 0.3 }
  },
  /** Night sea under a bright moon and stars (stage "nightfleet"). */
  nightfleet: {
    ...oceanDay,
    elev: 20,
    azim: -14,
    ...
    mie: 0.7,
    exposure: 0.08,
    sunTint: [0.7, 0.88, 1.2],
    groundAlbedo: [0.01, 0.02, 0.05],
    cloudCover: 0.2,
    cloudDensity: 0.6,
    cirrus: 0.2,
    stars: 1,
    ...
    envIntensity: 2.2,
    hemi: 0.6,
    sky: sky({ saturation: 1.3, zenithBoost: 0, hue: [0.05, 0.3, 1.0], skyHue: 0.7, horizonHue: 0.8, sunDisc: 1.2, sunGlow: 0.6 }),
    bloom: { threshold: 1.0, smoothing: 1.5, intensity: 1.0 },
    flare: 0.3,
    clouds: { count: 0.5, minY: 300, maxY: 1300, spread: 2600, puffSize: [180, 400], sun: 0.6, ambient: 0.8 }
  },
  /** Snowfield in a bright white haze (stage "whiteout", valley terrain). */
  whiteout: {
    ...emerald,
    elev: 32,
    azim: -12,
    rayleigh: 1.2,
    mie: 1.3,
    groundAlbedo: [0.6, 0.66, 0.72],
    cloudCover: 0.45,
    fogDensity: 0.00012,
    fogFalloff: 0.0012,
    horizonFog: 0.3,
    ocean: null,
    grade: 'arcadeWhiteout',
    toneExposure: 1.0,
    sky: sky({ saturation: 1.25, hue: [0.1, 0.5, 1.0], horizonHue: 0.9, horizonBright: 0.2 }),
    clouds: { ...emerald.clouds, count: 0.8 }
  },
  /** Narrow canyon at dusk, magenta sky (stage "ravine"). */
  ravine: {
    ...canyonRed,
    elev: 8,
    azim: 12,
    rayleigh: 1.5,
    mie: 1.4,
    groundAlbedo: [0.32, 0.14, 0.14],
    fogDensity: 0.0001,
    horizonFog: 0.3,
    grade: 'arcadeRavine',
    toneExposure: 1.05,
    sunTint: [1.1, 0.9, 0.85],
    flare: 0.6,
    sky: sky({ saturation: 1.4, zenithBoost: -0.2, hue: [0.9, 0.3, 0.55], skyHue: 0.6, horizonHue: 0.5, knee: 0.3, sunDisc: 2.2, sunGlow: 1.3 })
  },
  /** Pink dawn above the cloud deck (stage "jetstream"). */
  jetstream: {
    ...clouds,
    elev: 9,
    azim: -28,
    rayleigh: 1.45,
    mie: 1.0,
    grade: 'arcadeJetstream',
    toneExposure: 0.95,
    sunTint: [1.05, 0.95, 0.9],
    sky: sky({ saturation: 1.35, zenithBoost: -0.1, hue: [0.95, 0.5, 0.65], skyHue: 0.6, horizonHue: 0.5, knee: 0.3, sunDisc: 2.0, sunGlow: 1.1 })
  },
  /** Moonlit desert, silver-blue sand (stage "badlands", dunes terrain). */
  badlands: {
    ...canyonRed,
    elev: 24,
    azim: 16,
    ...
    mie: 0.7,
    exposure: 0.09,
    sunTint: [0.72, 0.88, 1.2],
    groundAlbedo: [0.1, 0.1, 0.14],
    cloudCover: 0.1,
    stars: 1,
    aurora: 0,
    fogDensity: 0.00005,
    horizonFog: 0.14,
    grade: 'arcadeBadlands',
    toneExposure: 1.15,
    envIntensity: 2.0,
    hemi: 0.6,
    sky: sky({ saturation: 1.3, zenithBoost: 0, hue: [0.05, 0.35, 1.0], skyHue: 0.7, horizonHue: 0.8, sunDisc: 1.2, sunGlow: 0.6 }),
    bloom: { threshold: 1.0, smoothing: 1.5, intensity: 1.0 },
    flare: 0.25,
    clouds: null
  },
  /** Edge of space: deep blue-black sky, stars by day, the cloud deck far below (bonus stage "stratos"). */
  stratos: {
    ...clouds,
    elev: 30,
    azim: 10,
    ...
    toneExposure: 1.0,
    flare: 0.8,
    sky: sky({ saturation: 1.4, zenithBoost: 0.1, hue: [0.05, 0.2, 1.0], skyHue: 0.8, horizonHue: 0.6, sunDisc: 1.8, sunGlow: 1.0 }),
    deck: { y: 1050, cover: 0.8, sun: 0.6 },
    clouds: null
  },
  /** Title flyby. */
